...
def analyze_complex(lat: float, lon: float):
    ...
    prompt = f"""
    Выступай в роли специалиста по пространственной эпидемиологии. Локация: {loc_info['name']}.
    Данные (Городской Экспосом):
    1. LST (Температура): {round(heat_val or 0, 1)} °C
    2. NO2 (приземный): {round(c_surf_mg_m3, 3)} мг/м³
    3. Шум: {round(noise_val or 0, 1)} дБА
    4. Свет: {round(light_val or 0, 1)} nW/cm²/sr
    
    Напиши научное заключение о СИНЕРГЕТИЧЕСКОМ влиянии этих 4 факторов на здоровье. 
    Обязательно свяжи их логически (например: LST + NO2 = риск фотохимического смога и озона; Шум + Свет = нейроэндокринный стресс). 
    Отвечай вариабельно, опираясь строго на цифры. Если все идеально - опиши феномен салютогенеза среды.
    Один абзац. Без советов. Без воды. До 100 слов.
    """
